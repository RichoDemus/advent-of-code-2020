...
fn part1(input: &str) -> usize {
    let rules = input.lines().map(line_to_bag_rule)
    ...
    rules.keys()
        .map(|bag| {
            let can = can_contain_golden_bag_rec(&rules, &bag);
            println!("{:?} can contain a golden bag? {}", bag,can);
            can
        })
...
fn can_contain_golden_bag_rec(bags: &HashMap<String, Vec<Content>>, current_bag: &String) -> bool {
    let empty_vec = vec![];
    ...
    if bags_inside.iter().any(|bag|bag.color == "shiny gold".to_string()) {
        return true;
    }

    bags_inside.into_iter()
        .map(|bag| {
            let can = can_contain_golden_bag_rec(bags, &bag.color);
            // println!("{:?} has an {:?} which can contain a golden bag: {}",current_bag, bag.color, can);
            can
        })
...
fn get_bags_inside_rec(bags: &HashMap<String, Vec<Content>>, current_bag: &String) -> usize {
    let empty_vec = vec![];
    let bags_inside = bags.get(current_bag).unwrap_or(&empty_vec);

    let inside: usize = bags_inside.iter().map(|content|get_bags_inside_rec(bags, &content.color) * content.amount).sum();
    let result = 1 + inside;
    println!("{:?} contain {} bags", current_bag, inside);
    return result
}
...
mod tests {
    ...
    fn test_parse_single_line_one_contents() {
        ...
        assert_eq!(result,
                   Rule {
                       ...
                       contents: vec![
                           Content { amount: 1, color: "shiny gold".to_string()},
                       ]
                   }
        );
    }
    ...
    fn test_parse_single_line_multiple_contents() {
        let result = line_to_bag_rule("light red bags contain 1 bright white bag, 2 muted yellow bags.");

        assert_eq!(result,
                   Rule {
                       color: "light red".to_string(),
                       contents: vec![
                           Content { amount: 1, color: "bright white".to_string()},
                           Content { amount: 2, color: "muted yellow".to_string()},
                       ]
                   }
        );
    }
